...
\section{Additionneur 8 bits}
   ...
   \begin{tcolorbox}[colframe=Monokaimagenta,colback=white]
		...
		\paragraph{}
		L'additionneur 8bits est composé de 2 additionneurs 4bits qui sont eux composés d'aditionneurs 1bit, cette séparation  par couche  nous a permis de comprendre le fonctionnement à la base d'un aditionneur pour ensuite le transposer en 8bits.
		\paragraph{}
...
\section{Vérification d'intégrité en réception}
	...
	\begin{tcolorbox}[colframe=Monokaimagenta,colback=white]
		...
		\paragraph{}
		Ce circuit permet de contrôler l'intégrité d'une donnée représentée ici avec 3 octets et 1 checksum pour assurer que la transmission s'est bien déroulée. Comme le checksum représente la valeur qui additionnée avec les valeurs d'entrées vaut
...
\section{Vérification du 2ième groupe de données}
	...
	\begin{tcolorbox}[colframe=Monokaimagenta,colback=white]
		...
		Obtention du checksum. Le calcul se fait avec les données 0xA5, 0x9E et 0x6F afin d'obtenir le résultat correspondant à ces données.
			...
			Tout les overflow ne vont être valide du point de notre vérification, 
			mais le fait que c'est possible nous montre la limitation de ce système.
			(on propose en annexe une possiblité de résolution de ce problème lors du 
			checksum.)
	...
	\section{Annexe}
		...
		En rajoutant une sortie   \textbf{overflow} à notre fonction checksum, on peut savoir si la somme
		de nos valeurs est supérieure à la valeur maximum enregistrable sur le nombre de 
		bits que nous avons. \\
		De garder cette information nous permet de savoir que même si notre checksum est 
		\textit{toujours} valable (dans le sens que la somme des 3 valeurs et du checksum
		sera égal à 0), il y un autre set de valeur qui donne 0 additionné avec ce checksum.
		Nous n'avons donc pas une garantie parfaite de détection d'erreur dans notre fonction
		de base, l'utilisation de cette nouvelle fonction plus complète permettrait d'être
		plus précis dans la gestion des erreurs. 
		\end{tcolorbox}
\end{document}
